...
export function loadAppendSuccessorProtocolHarness() {
    ...
    const transitionEnd = source.indexOf('function ensureThinkingUnique', transitionStart);
    ...
    const context: any = { Map, Set, Array, Object, console, activeSessionId: 'ses_f8', getSessionState: (sessionId: string) => sessions.get(sessionId), vscode: { postMessage: (message: any) => posts.push(message) }, sessionSearch: { rekey: () => undefined }, subagentTextExpandedByKey: new Map(), logTimelineSnapshot: () => undefined, syncAppendSnapshotMetadata: () => undefined };
