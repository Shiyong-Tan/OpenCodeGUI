...
export class SidebarProvider implements vscode.WebviewViewProvider {
    ...
    private consumeAppendSuccessorTmpKey(sessionId: string, successor: { generation: number; sealedPredecessorTmpKey?: string }): void {
        const expected = successor?.sealedPredecessorTmpKey; const actual = this.pendingAssistantTmpKeyBySession.get(sessionId);
        if (!expected || !actual || actual !== expected) { this.uiDebugChannel.appendLine(`[EXT][APPEND_SUCCESSOR_TMP_RETAIN] sessionId=${sessionId} generation=${successor?.generation} reason=key-mismatch-or-missing`); return; }
        this.pendingAssistantTmpKeyBySession.delete(sessionId); this.uiDebugChannel.appendLine(`[EXT][APPEND_SUCCESSOR_TMP_CLEAR] sessionId=${sessionId} generation=${successor.generation}`);
    }
