...
export async function handleSidebarChatEvent(
    ...
    webview: vscode.Webview
        ...
        if (event.type === 'turnResolved' && event.sessionId) {
            ...
            if (appendSuccessor) {
                host.uiDebugChannel.appendLine(`[EXT][APPEND_SUCCESSOR_FINAL] sessionId=${event.sessionId} rootUserMsgId=${appendSuccessor.rootUserMsgId} appendUserMsgId=${appendSuccessor.appendUserMsgId} successorAssistantMsgId=${appendSuccessor.assistantMsgId} generation=${appendSuccessor.generation} outcome=success`);
            }
            return;
        }
