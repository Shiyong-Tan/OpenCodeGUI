...
export function mapServerEventToChatEvents(
    ...
    source: EventSource = 'sse'
        ...
        let appendSuccessorResult: any;
        if (type === 'message.updated' && props?.info?.role === 'assistant') appendSuccessorResult = host.bindAppendSuccessor(typeof props.info.sessionID === 'string' ? props.info.sessionID : undefined, typeof props.info.id === 'string' ? props.info.id : undefined, typeof props.info.parentID === 'string' ? props.info.parentID : undefined);
        ...
        const activeAppendSuccessor = host.getActiveAppendSuccessor?.(sessionId);
        if (source === 'sse' && activeAppendSuccessor && (type === 'message.updated' || type === 'message.part.updated') && !exactAppendSuccessor) return events;
        if (appendSuccessorResult?.status === 'new' && sessionId) events.push({ type: 'turnInFlight', sessionId, inFlight: true, ownerMsgId: appendSuccessorResult.binding.assistantMsgId, assistantMsgId: appendSuccessorResult.binding.assistantMsgId, appendSuccessor: appendSuccessorResult.binding, source });
        if (sessionId) {
