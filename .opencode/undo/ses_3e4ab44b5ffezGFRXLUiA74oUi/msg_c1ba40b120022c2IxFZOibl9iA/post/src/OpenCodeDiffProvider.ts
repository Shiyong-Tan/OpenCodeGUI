...
export class OpenCodeDiffProvider implements vscode.TextDocumentContentProvider {
    ...
    public async applyUnifiedDiff(filePath: string, diffText: string): Promise<void> {
        ...
        if (state.baseline === state.current) {
            const derived = this.tryDeriveBaseline(next, patchText);
            if (derived !== undefined) {
                state.baseline = derived;
            }
        }
    ...
    public async applyWorkspaceSnapshot(filePath: string, patchText: string): Promise<void> {
        ...
        if (state.baseline === state.current) {
            const derived = this.tryDeriveBaseline(next, patchForFile);
            if (derived !== undefined) {
                state.baseline = derived;
            }
        }
    ...
    private async applyPatchOrSnapshot(filePath: string, current: string, patchText: string): Promise<string> {
        ...
        if (!this.workspaceRoot) return current;
        const absPath = path.isAbsolute(filePath) ? filePath : path.join(this.workspaceRoot, filePath);
        return fs.existsSync(absPath) ? await fs.promises.readFile(absPath, 'utf-8') : current;
    }

    private tryDeriveBaseline(current: string, patchText: string): string | undefined {
        if (!patchText.includes('@@')) return undefined;
        const lines = patchText.split('\n');
        const inverted = lines.map((rawLine) => {
            const line = rawLine.endsWith('\r') ? rawLine.slice(0, -1) : rawLine;
        ...
        return this.tryApplyPatch(current, inverted);
    }
